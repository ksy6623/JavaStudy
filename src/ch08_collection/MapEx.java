...
public class MapEx {
	public static void main(String[] args) {
		...
		// 가수정보만 출력(중복 x)
		HashSet<String> uniqueSubgers = new HashSet<String>();
		for(int i = 0; i < playList.size(); i++) {
			String [] album = playList.get(i).split(":");
			String singer = album[1];
			uniqueSubgers.add(singer);
		}
		System.out.println(uniqueSubgers);
		System.out.println(uniqueSubgers.size());
		// 가수 이름을 오름차순으로 정렬
		ArrayList<String> uniquList = new ArrayList<String>(uniqueSubgers);
		Collections.sort(uniquList);
...
//
//		// 입력받은 가수의 노래 목록 출력
//		if (artistSongMap.containsKey(selectedArtist)) {
//		    System.out.println("\n" + selectedArtist + "의 노래 목록:");
//		    ArrayList<String> songs = artistSongMap.get(selectedArtist);
